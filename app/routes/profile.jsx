...
export default function Profile() {
    ...
    return (
        <div className="flex flex-col items-center justify-start h-screen bg-gray-100 p-10">
            ...
            <h1 className="text-2xl font-bold mb-4 py-2">Events oprettet af dig</h1>
            <ul className="list-disc list-inside">


                    {user.events.map((event, index) => (
                        <li key={index} className="mb-2">
                          <a className="font-semibold text-lg hover:text-gray-500" 
                          href={`/events/${event._id}`}>{event.title}<br></br>{event.location}</a>
                ...
                <div className="w-1/2 pr-2">
                <h2 className="text-2xl font-semibold mb-4 py-2">Events du er tilmeldt</h2>
                <ul className="list-disc list-inside">


                {user.registeredEvents && user.registeredEvents.map((event, index) => (
                        <li key={index} className="mb-2">
