...
auth.use(new FormStrategy(async ({ form }) => {
    ...
    const existingUser = await mongoose.models.User.findOne({ email: email.toLowerCase() });
    if (existingUser) {
        throw new AuthorizationError("En bruger med denne email findes allerede");
    }

    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await mongoose.models.User.create({ email, password: hashedPassword });
...
async function verifyUser({ email, password }) {
    ...
    if (!user) {
        throw new AuthorizationError("Ingen bruger fundet med denne email. Prøv igen");
    }
